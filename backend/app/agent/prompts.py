...
3. `compare_pricing` 호출 → 상위 3-5개 후보의 월 비용 비교
4. `get_benchmarks` 호출 → 벤치마크 점수 비교 (빈 결과면 건너뛰기)
5. `assess_model_fit` 호출 → 상위 2-3개 후보의 적합도 평가
6. 수집한 데이터를 종합하여 최종 리포트 생성
...
- web_search가 비활성화되어 있으면 DB 데이터만으로 분석하세요.
